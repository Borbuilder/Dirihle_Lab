...
namespace LAB_CHM
{
    public partial class Form4 : Form
    {
        ...
        int N;
        int M;
        public Form4(double[][] u, double[][] v1, int n, int m, bool main)
        {
            InitializeComponent();
            ...
            if (main == false)
            {
                ...
                cPoint3D[,] i_Points3D = new cPoint3D[u.Length, u[0].Length];
                for (int X = 0; X < u.Length; X++)
                {
                    for (int Y = 0; Y < u[0].Length; Y++)
                    {
                        i_Points3D[X, Y] = new cPoint3D(X * 1.0 / (double)n, Y * 1.0 / (double)m, u[X][ Y]);
                    }
                }

                cPoint3D[,] i_Points3D2 = new cPoint3D[v1.Length, v1[0].Length];
                for (int X = 0; X < v1.Length; X++)
                {
                    for (int Y = 0; Y < v1[0].Length; Y++)
                    {
                        i_Points3D2[X, Y] = new cPoint3D(X * 1.0 / (double)n, Y * 1.0 / (double)m, v1[X][Y]);
                    }
                }
            ...
            else
            {
                ...
                for (int X = 0; X < v1.Length; X++)
                {
                    for (int Y = 0; Y < v1[0].Length; Y++)
                    {
                        i_Points3D2[X, Y] = new cPoint3D(X * 1.0 / (double)n*0.5, Y * 1.0 / (double)m*0.5, v1[X][Y]);
                    }
                }
                graph3D1.AxisX_Legend = "X from 0 to 1";
                graph3D1.AxisY_Legend = "Y from 0 to 1";
                ...
                for (int i = 0; i < n + 1; i++)
                {
                    for (int j = 0; j < m + 1; j++)
                    {
                        ARR[i, j] = Math.Abs(u[i][j] - v1[i*2][j*2]);
                    }
                };
                N = n;
                M = m;
